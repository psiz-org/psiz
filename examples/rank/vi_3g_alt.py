...
def main():
    """Run script."""
    # Settings.
    fp_example = Path.home() / Path('psiz_examples', 'rank', 'vi_3g_alt')
...
def build_model(n_stimuli, n_dim, n_group, kl_weight):
    ...
    # Define group-specific kernels with a shared similarity layer.
    # shared_similarity = psiz.keras.layers.ExponentialSimilarity(
    #     tau_initializer=tf.keras.initializers.Constant(1.),
    #     gamma_initializer=tf.keras.initializers.Constant(0.),
    #     trainable=False
    # )
    kernel_0 = build_kernel(kl_weight)
    kernel_1 = build_kernel(kl_weight)
    kernel_2 = build_kernel(kl_weight)
    kernel_group = psiz.keras.layers.GroupSpecific(
        [kernel_0, kernel_1, kernel_2], group_col=1
    )
    kernel_adapt = psiz.keras.layers.KernelAdapter(
        kernel=kernel_group
    )

    model = psiz.keras.models.Rank(
        stimuli=stimuli, kernel=kernel_adapt, n_sample=1
    )
    # TODO test submodules listing, is shared similarity multiple?
    # Maybe a test for injecting shared layers?
    return model


def build_kernel(kl_weight):
    ...
    mink_variational = psiz.keras.layers.MinkowskiVariational(
        posterior=mink_posterior,
        prior=mink_prior,
        kl_weight=kl_weight, kl_n_sample=30
    )
    kernel = psiz.keras.layers.DistanceBased(
        distance=mink_variational,
