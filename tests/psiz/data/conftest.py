...
def o_2rank1_aa_4x1():
    ...
    sample_weight = 0.9 * np.ones([4, 1])
    n_outcome = 2
    rank_outcome = SparseCategorical(
        outcome_idx, depth=n_outcome, sample_weight=sample_weight, name="rank_prob"
    )
    return rank_outcome
...
def o_rt_a_3x2():
    ...
    sample_weight = 0.8 * np.ones([3, 2])
    return Continuous(rt, sample_weight=sample_weight, name="rt")
...
def c_rate2_a_4x1():
    ...
    stimulus_set = np.array([[3, 1], [9, 12], [3, 4], [3, 4]], dtype=np.int32)
...
def c_categorize_a_4x10():
    ...
    objective_query_label = to_categorical(objective_query_label, num_classes=3)
    content = Categorize(
...
def o_continuous_a_4x1():
    ...
    outcome = np.array([[0.0], [2.0], [-0.1], [1.3]], dtype=np.float32)
...
def o_continuous_e_4x3():
    ...
    * with sample weights
    ...
    return Continuous(outcome, name="continuous_e", sample_weight=sample_weight)


@pytest.fixture(scope="module")
def o_sparsecat_a_4x1():
    """Outcome SparseCategorical with minimal rank arguments."""
    outcome_idx = np.array([0, 2, 0, 1], dtype=np.int32)
...
def g_condition_idx_4x1():
    ...
    return Group(value, name="condition_idx")
...
def g_condition_label_4x1():
    ...
    return Group(value, name="condition_label")
...
def g_condition_idx_3x2():
    value = np.array([[[0], [0]], [[1], [1]], [[0], [0]]], dtype=np.int32)
    return Group(value, name="condition_idx")
